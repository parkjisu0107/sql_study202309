...
FROM
    ...
    FROM
        (SELECT * FROM employees ORDER BY hire_date)tbl)
...
*/
SELECT
    e.employee_id, CONCAT(first_name, last_name) AS full_name,
...
*/
SELECT 
    e.employee_id, CONCAT(first_name, last_name) AS full_name, e.department_id, 
...
SELECT
     ...
     (SELECT loc.street_address
      FROM locations loc WHERE d.location_id = loc.location_id) AS street_address,
      (SELECT loc.postal_code
      FROM locations loc WHERE d.location_id = loc.location_id) AS postal_code,
      (SELECT loc.city
      FROM locations loc WHERE d.location_id = loc.location_id) AS city
FROM departments d
ORDER BY department_id;
...
*/
SELECT
    loc.location_id, loc.street_address, loc.city, c.country_id, c.country_name
FROM locations loc
...
SELECT *
FROM 
    (SELECT ROWNUM rn, tdl.*
    FROM
        (
        SELECT
            e.employee_id, e.first_name,
            e.phone_number, e.hire_date, d.department_id, d.department_name
        FROM employees e
        LEFT JOIN departments d
        ON e.department_id = d.department_id
        ORDER BY hire_date
